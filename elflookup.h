			   ...
			   char *section)
{
	...
	fread_result = fread(buff, 1, sh_table[eh.e_shstrndx].sh_size, fd);
    }
    sh_str = buff;
	...
	fread_result = fread(res, 1, sh_table[i].sh_size, fd);
	return res;
    }
...
char *get_elf_section_file(char *section, char *filename)
{
    ...
    fseek(fp, 0, SEEK_SET);
    fread_result = fread(&elf_header, 1, sizeof(Elf64_Ehdr), fp);
    sh_table = malloc(elf_header.e_shentsize * elf_header.e_shnum);

    fseek(fp, elf_header.e_shoff, SEEK_SET);
    fread_result = fread(sh_table, 1, elf_header.e_shentsize * elf_header.e_shnum, fp);
